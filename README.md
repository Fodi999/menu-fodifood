# 👨‍🍳 Профессиональное Резюме# 🍣 FODI SUSHI - Интернет-магазин доставки суши



Современный одностраничный сайт-резюме для профессионального повара, созданный с использованием передовых веб-технологий.Современный веб-сайт для заказа и доставки премиальных суши и роллов, построенный на Next.js 15 с поддержкой мультиязычности и Go backend.



## 🚀 Технологии![FODI SUSHI](https://img.shields.io/badge/FODI-SUSHI-orange?style=for-the-badge)

![Next.js](https://img.shields.io/badge/Next.js-15.5.4-black?style=for-the-badge&logo=next.js)

- **Next.js 15** - App Router, React Server Components![React](https://img.shields.io/badge/React-19.2.0-61dafb?style=for-the-badge&logo=react)

- **React 19** - Последняя версия![TypeScript](https://img.shields.io/badge/TypeScript-5.0-blue?style=for-the-badge&logo=typescript)

- **TypeScript 5** - Типобезопасность![Tailwind CSS](https://img.shields.io/badge/Tailwind-4.0_beta-38bdf8?style=for-the-badge&logo=tailwindcss)

- **Tailwind CSS 4** - Утилитарный CSS![Go](https://img.shields.io/badge/Go-1.21-00ADD8?style=for-the-badge&logo=go)

- **shadcn/ui** - Современные компоненты![PostgreSQL](https://img.shields.io/badge/PostgreSQL-Neon-336791?style=for-the-badge&logo=postgresql)

- **Framer Motion** - Плавные анимации

- **next-themes** - Светлая/тёмная тема## 🌐 Production URLs



## ✨ Особенности- **Frontend:** https://menu-fodifood.vercel.app

- **Backend API:** https://irrelevant-nellie-fodi999-aefe2c9f.koyeb.app

- ✅ Полностью адаптивная вёрстка (mobile, tablet, desktop)- **Database:** Neon PostgreSQL (Serverless)

- ✅ Светлая/тёмная тема с плавным переключением

- ✅ Плавные micro-interaction анимации## ✨ Основные возможности

- ✅ SEO оптимизация (meta tags, OG tags)

- ✅ Минимализм + современный tech-дизайн### 🎯 Для клиентов

- ✅ Быстрая загрузка (Server Components)- 🍱 **Каталог продуктов** - роллы, суши, сеты с подробным описанием

- ✅ Lazy loading изображений- 🛒 **Корзина покупок** - добавление товаров, управление количеством

- ✅ Smooth scrolling между секциями- 📝 **Форма заказа** - оформление заказа с указанием данных доставки

- 🌐 **Мультиязычность** - поддержка 3 языков (EN, RU, PL) с 111+ переводами

## 🚀 Быстрый старт- 🔐 **Аутентификация** - вход, регистрация через Go Backend (JWT)

- 👤 **Личный кабинет** - история заказов, управление профилем

```bash

# Установка зависимостей### 🔧 Для администраторов

npm install- **🛒 Единая корзина компонентов v2.0** — новая система управления продуктами:

  - 🥬 Добавление ингредиентов с фильтрацией по категориям

# Запуск в режиме разработки  - 📦 Добавление полуфабрикатов из готового списка

npm run dev  - 🧮 Автоматический расчёт себестоимости, веса, наценки

  - 🎨 Визуальная индикация типов компонентов (зелёный/фиолетовый)

# Сборка для продакшена  - 🔥 Поддержка термообработки с ручным вводом веса

npm run build  - ✅ Валидация и автоформатирование до 3 знаков

- 👥 **Управление пользователями** - просмотр, редактирование, блокировка

# Запуск продакшен версии- 📦 **Управление заказами** - обработка, статусы, история

npm start- 🏪 **Управление товарами** - добавление, редактирование, удаление

```- 📊 **Аналитика** - статистика продаж и популярности товаров



Откройте [http://localhost:3000](http://localhost:3000)### ⚙️ Технологии

- 💾 **База данных** - Prisma ORM + PostgreSQL (Neon)

## 📖 Документация- 🔌 **Go Backend API** - REST API на Go с JWT авторизацией

- 📱 **Адаптивный дизайн** - идеально работает на всех устройствах

- **START-HERE.md** - начните отсюда! 🎯- 🎨 **Современный UI/UX** - темная тема с иконками lucide-react

- **RESUME-README.md** - полное руководство- ⚡ **Быстрая сборка** - Turbopack для мгновенной разработки
...
│   ├── providers.tsx     # Theme + Toaster### Настройка переменных окружения
...
│   ├── Resume/           # Компоненты резюме```bash

│   ├── ThemeToggle.tsx   # Переключатель темыcp .env.example .env.local

│   └── ui/               # shadcn/ui компоненты```

│

├── lib/#### 2. Отредактируйте `.env.local` для локальной разработки:

│   └── utils.ts          # Утилиты

│```env

└── types/# Локальный PostgreSQL

    └── css.d.ts          # CSS типыDATABASE_URL="postgresql://user:password@localhost:5432/fodisushi"

```

# Локальный сервер

## 🎨 КастомизацияNEXTAUTH_URL="http://localhost:3000"



### Изменить личную информацию# Сгенерируйте секрет

NEXTAUTH_SECRET="your_generated_secret"

Отредактируйте компоненты в `src/components/Resume/`:```

- `HeroSection.tsx` - имя, профессия, описание

- `ContactSection.tsx` - контакты#### 3. Сгенерируйте `NEXTAUTH_SECRET`:



### Добавить свои фотографии```bash

openssl rand -base64 32

``````

/public/avatar.jpg         # Ваше фото (400x400px)

/public/portfolio/*.jpg    # Фото блюд (800x800px)Скопируйте результат и вставьте в `.env.local`.
...
Создано с ❤️ используя Next.js```bash
